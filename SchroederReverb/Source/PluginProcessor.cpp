...
{
    //default values for plugin parameters, 50% mix, 3second RT60
...
void SchroederReverbAudioProcessor::setParameter (int index, float newValue)
{
    ...
    //value back through to the underlying Schroeder classes to update
...
void SchroederReverbAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    //get the mix parameter percentage from the UI
    float mix = getParameter(Mix)/100;
    
    // audio processing...left and right channels
